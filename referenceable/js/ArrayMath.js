class ArrayMath {
	...
	static mode = (array) => {
		...
		for (const value of Object.keys(counts)) {
			if (mostFrequentValue == null) { mostFrequentValue = value; }
			if (counts[value] > counts[mostFrequentValue]) { mostFrequentValue = value; }
		}
	...
	static standardDeviation = (array) => {
		return Math.sqrt( this.sum(array.map(x => Math.pow(x - this.average(array), 2))) / array.length );
	}
	...
	static largestValues = (array, amount) => {
		...
		if (amount >= array.length) { return sortedArray; }
	...
	static cumulativeSum = (array) => {
		let cumulativeSum = 0;
		const cumulativeSumArray = array.map((value) => { cumulativeSum += value; return cumulativeSum; });
	...
	static cumulativeProduct = (array) => {
		...
		const cumulativeProductArray = array.map((value) => { cumulativeProduct *= value; return cumulativeProduct; });
